    ...
    try:
        ...
        for i in range(0, len(documents), batch_size):
            ...
            try:
                ...
                # Print the first document to help debug
                if batch:
                    logger.error(f"First document in batch: {json.dumps(batch[0], default=str)[:200]}...")
...
def process_blob_data(blob_storage: BlobStorage, blob_name: str, embeddings_generator: EmbeddingsGenerator = None) -> List[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        embeddings_generator: Optional EmbeddingsGenerator instance for generating embeddings
    ...
    for i, chunk in enumerate(blob_data):
        ...
        if not metadata:
            logger.warning(f"Chunk {i} in {blob_name} has no metadata, skipping")
            continue
        
        # Extract or generate embedding
        embedding = chunk.get('embedding', [])
        if not embedding and embeddings_generator:
            # Generate embedding if not present and generator is provided
            content_to_embed = metadata.get('content_to_embed', content)
            embedding = embeddings_generator.generate_embedding(content_to_embed)
            if not embedding:
                logger.warning(f"Failed to generate embedding for chunk {i} in {blob_name}")
                continue
        elif not embedding:
            logger.warning(f"Chunk {i} in {blob_name} has no embedding and no generator provided, skipping")
            continue
        
        # Create document ID (use id from metadata if available, otherwise generate one)
        doc_id = metadata.get('id', f"{blob_name.replace('.json', '')}_{i}")
        ...
        if other_metadata:
            document["metadata_json"] = json.dumps(other_metadata)
...
def create_and_populate_index_from_blobs(
    ...
    Args:
        ...
        max_blobs: Maximum number of blobs to process (for testing)
    ...
    if max_blobs and len(blob_names) > max_blobs:
        logger.info(f"Limiting to {max_blobs} blobs for processing")
    ...
    for blob_name in blob_names:
        ...
        documents = process_blob_data(blob_storage, blob_name, embeddings_generator)
...
def main():
    ...
    parser.add_argument('--max-blobs', type=int, help='Maximum number of blobs to process (for testing)')
