...
class KnowledgeAssistant:
    ...
    def _register_semantic_functions(self):
        ...
        You are an AI assistant analyzing user queries to determine their intent.
        ...
        Output your analysis as a JSON object with the following structure:
        {
            ...
            "confidence": 0.XX,
            "explanation": "Brief explanation of why this intent was selected",
            "parameters": {
                "param1": "value1",
                "param2": "value2"
            }
    ...
    async def process_query(self, query: str) -> str:
        ...
        try:
            ...
            # Debug the actual result format
            logger.info(f"Intent result type: {type(intent_result)}")
            logger.info(f"Intent result value: {intent_result}")
            
            try:
                # Try parsing as JSON - first clean up any code fence markers
                result_str = str(intent_result)
                ...
                intent_data = json.loads(result_str)
            except json.JSONDecodeError as e:
                # If not valid JSON, create a default structure
                logger.warning(f"Could not parse intent result as JSON: {intent_result}")
                logger.warning(f"JSON parsing error: {str(e)}")
                intent_data = {"intent": "TECHNICAL_QUESTION", "confidence": 0.8, "explanation": "Default intent due to parsing error"}
            intent = intent_data.get("intent")
            logger.info(f"Recognized intent: {intent} with confidence {intent_data.get('confidence')}")
            
            # 2. Process based on intent
            if intent == "TECHNICAL_QUESTION":
                return await self._process_technical_question(query)
            elif intent == "ISSUE_CREATION":
                return await self._process_issue_creation(query)
            elif intent == "STATUS_REPORT":
                return await self._process_status_report(query, intent_data.get("parameters", {}))
            elif intent == "AUTHENTICATION":
                return await self._process_authentication(query, intent_data.get("parameters", {}))
            else:  # GENERAL_QUERY
    ...
    async def _process_technical_question(self, query: str) -> str:
        """
        ...
        # 1. Retrieve relevant information from search index
        context = ""
    ...
    async def _process_status_report(self, query: str, parameters: Dict[str, Any]) -> str:
        """
